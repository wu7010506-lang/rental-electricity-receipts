...
function App() {
  ...
  return <main><header><div><p className="eyebrow">RENTAL UTILITY DESK</p><h1>每月帳單，一張繳款單就完成。</h1><p>輸入每個房間的本月電表讀數，再依房號立即產生租金與電費繳款單。</p></div><div className="month">帳單月份<strong>7 月</strong></div></header><nav><button className={tab==="receipt"?"on":""} onClick={()=>setTab("receipt")}>租金繳款單</button><button className={tab==="ledger"?"on":""} onClick={()=>setTab("ledger")}>本月用電資料</button><button className={tab==="august115"?"on":""} onClick={()=>setTab("august115")}>下個月用電資料</button></nav>{tab==="receipt"?<section className="workspace"><aside><b>查詢房號</b><input list="rooms" value={room} onChange={e=>setRoom(e.target.value)} /><datalist id="rooms">{tenants.map(t=><option key={t.room} value={t.room}/>)}</datalist><p>輸入房號後，繳款單會立即更新。</p></aside><article className="receipt"><div className="head"><div><small>7 月房租</small><h2>租金繳款單</h2></div><b>{tenant?.room||"—"}</b></div><div className="owner"><span>房東</span><b>姜 義 彬</b><span>帳戶</span><b>華南銀行 南永和分行</b><span>帳號</span><b>169-20-0147047</b><span>戶名</span><b>吳金泉</b><span>聯絡電話</span><b>0955902392（LINE ID: may0955902392）</b></div>{tenant?<div className="charges"><p><span>月租金</span><b>NT$ {fmt(tenant.rent)}</b></p><p><span>用電度數</span><b>{fmt(tenant.current)} − {fmt(tenant.previous)} ＝ {fmt(usage)} 度</b></p><p><span>電費</span><b>NT$ {fmt(fee)}</b></p><p className="total"><span>合計</span><b>NT$ {fmt(total)}</b></p></div>:<p>找不到此房號。</p>}<button className="print" onClick={()=>window.print()}>列印此繳款單</button></article></section>:tab==="ledger"?<UsageTable rows={tenants} setRows={setTenants} heading="本月用電資料" description="更新讀數後，電費與合計會無條件捨去至整數元。"/>:<UsageTable rows={august115} setRows={setAugust115} sourceRows={tenants} heading="下個月用電資料" description="上月用電會自動帶入本月用電資料中的本月用電；請輸入新的本月用電以計算電費與合計。" showContacts/>}<footer>電費以「本月用電 − 上月用電」計算；電費與合計無條件捨去至整數元。</footer></main>;
}
